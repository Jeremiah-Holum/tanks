...
try {
  ...
  await page.evaluate(() => {
    ...
    S._spike = (cpu, sim, ai, render, hud, audio) => {
      ...
      const st = Math.round((S.world.time - (S._tPrev ?? S.world.time)) * 60); S._tPrev = S.world.time;
      recs.push({ t: S.world.time, st, cpu, js: cpu - gl, gl, sim, ai, tanks: V.tanks || 0, fx: (V.ev || 0) + (V.fx || 0), hud, audio, progs: V.progs || 0, ev });
    };
  });
  ...
  const f = (r) => r && r.js ? `${r.js.toFixed(1)} ms JS, ${(r.tanks + r.fx + r.hud + r.audio).toFixed(1)} ms view+hud+audio (${r.st} steps: sim ${r.sim.toFixed(1)} ai ${r.ai.toFixed(1)} tanks ${r.tanks.toFixed(1)} fx ${r.fx.toFixed(1)} hud ${r.hud.toFixed(1)} audio ${r.audio.toFixed(1)}; gl ${r.gl.toFixed(0)}) ev ${JSON.stringify(r.ev)}` : '-';
  ...
  for (const r of after) r.va = r.tanks + r.fx + r.hud + r.audio;
  console.log(`worst view+hud+audio: spot ${max(spot, 'va').va?.toFixed(1)} ms, volley ${max(volley, 'va').va?.toFixed(1)} ms, overall ${f(max(after, 'va'))}`);
