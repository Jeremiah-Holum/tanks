...
const click = async (sel) => {
  ...
  catch (e) { if (!/Timeout/.test(e.message)) throw e; log('note: forced click on', sel); await page.locator(sel).first().scrollIntoViewIfNeeded(); await page.click(sel, { timeout: 15000, force: true }); }
};
const shot = async (name) => { if (shotDir) { await ev(() => window.__tt.renderOnce()); await page.screenshot({ path: `${shotDir}/${q}-${name}.png`, timeout: 180000 }); } };
...
async function toFight() { await tick(0.5, 6); } // the mission banner runs 2.4 s of game time
...
await click('[data-seg="mycls"] button[data-val="td"]');
